...
import { createCloudflare, PAGES_PROJECT_LIMIT, PAGES_PROJECT_WARN_AT, RECORDED_ERRORS } from "../_lib/cf.mjs";
...
export async function runCheckCredentials({ env, logger, fetchImpl, random }) {
  ...
  // ── how much room is left on the account (issue #255) ──────────────────
  //
  // The count is already on the wire. `bindsToAccount` is a Pages project
  // listing, a Cloudflare v4 collection carries `result_info.total_count`, and
  // that field is what measured this account at 51/100 in issue #1. Reading it
  // here costs no extra request and lands at the EARLIEST step of the run,
  // before anything is built.
  //
  // It is a WARNING and never a failure, in both directions. An account over the
  // threshold must still deploy the sites it already has, and failing a run that
  // would have worked is the same class of error as reporting success on one
  // that did not. What it softens is a NEW site's project create at the ceiling
  // (ensure-project.mjs), which is a wall today and a slope with this line.
  //
  // Only a real number counts. Nobody here has measured this field on this exact
  // query, so its ABSENCE is an expected state rather than an error: a response
  // without it logs nothing at all. A string, a null or a NaN is left unread for
  // the same reason -- a headroom figure this step invented would be worse than
  // no headroom figure, and this is the step whose whole job is saying only what
  // it can see.
  const total = bound.json?.result_info?.total_count;
  if (typeof total === "number" && Number.isFinite(total)) {
    logger.info(`Pages projects on account ${accountId}: ${total} of Cloudflare's ${PAGES_PROJECT_LIMIT}.`);
    if (total >= PAGES_PROJECT_WARN_AT) {
      const room = Math.max(PAGES_PROJECT_LIMIT - total, 0);
      logger.warning(
        `Account ${accountId} holds ${total} of the ${PAGES_PROJECT_LIMIT} Cloudflare Pages projects an account is ` +
          `allowed, on every plan. Rocket Sites uses one project per client site, so ${room} more NEW site` +
          `${room === 1 ? "" : "s"} can be created on this account before the first deploy of the next one fails with ` +
          `nowhere to put it. THIS RUN IS UNAFFECTED and nothing is being stopped: a site that already has its ` +
          `project deploys into it whatever the count says. Free slots before the next new site: delete Pages ` +
          `projects nobody is using ('wrangler pages project list' shows each with its last deployment), or point ` +
          `the next site repository's CLOUDFLARE_ACCOUNT_ID at another Cloudflare account.`
      );
    }
  }
