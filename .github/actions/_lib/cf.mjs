...
/**
 * How many Pages projects one Cloudflare account may hold, and the point at
 * which this publisher starts saying so out loud (issue #255).
 *
 * `PAGES_PROJECT_LIMIT` is Cloudflare's cap, the same on Free and on Workers
 * Paid, and Rocket Sites spends one project per client site -- so an agency's
 * hundredth site is their last on one account. ARCHITECTURE.md, "Scale
 * ceiling", is where that decision lives; the sibling store accounts have hit
 * it for real.
 *
 * `PAGES_PROJECT_WARN_AT` is the number ARCHITECTURE.md already uses for "an
 * agency below roughly 80 clients never notices". Crossing it is a WARNING and
 * never a failure, and that is the whole of its design: an account at 95 must
 * still deploy the 95 sites it already has, and refusing a run that would have
 * worked is the same class of error as reporting success on one that did not.
 */
export const PAGES_PROJECT_LIMIT = 100;
