...
//   6. the Cloudflare Pages control entries at the root of the artifact are
//      decided in BOTH directions, whatever shape each has. Four of them
//      (`_worker.js`, `functions`, `_routes.json`, `_redirects`) must be
//      absent: two put a Worker in front of a client's site and one can forge
//      this publisher's own gate proof. `_headers` must be PRESENT and must be
//      the publisher's own bytes, because since #260 the publisher writes it.
//      See PAGES_CONTROL_ENTRIES and PUBLISHER_AUTHORED_ENTRIES below.
...
//  10. no foreign document framed inside a client's page, except an <iframe>
//      from one of three named embed hosts, over https:, sandboxed (issue #292,
//      _lib/scripting.mjs). Also: no `srcdoc` on any element.
//
// RULE 10 CLOSES A HOLE AND OPENS A SMALLER ONE, in that order, and the first
// half is the part to keep in mind. Until it landed this guard read <link> and
// <script> and NOTHING ELSE for a foreign subresource, so an <iframe> pointing
// anywhere at all published onto a client's live site with no complaint --
// while the prose around rule 4 talked about a site that must not "depend on a
// third-party host at runtime", and while templates/validate.mjs RS005 refused
// a foreign <iframe src> on the pages we author. The strict rule applied to our
// pages and the loose one to the pages a client actually gets. Issue #278 is
// the one that had this right; #292 said both enforcement points already
// refused an iframe, and only the edit guard did.
//
// The smaller half is the allowlist itself, which is a real widening of what a
// client's page may carry and is argued in ARCHITECTURE.md, invariant 17 and
// "Third-party embeds: three named players, and nothing else". What it costs:
// an allowlisted host is a third-party runtime dependency on a live client
// site, which is the thing rules 1, 4 and 5 exist to refuse for a stylesheet.
// The reason it is a different answer is that a stylesheet or a script executes
// IN the page's origin and can change all of it, and a cross-origin frame
// cannot: it is a rectangle that fails visibly and locally.
//
...
import { scriptingProblems, embedProblems } from "../_lib/scripting.mjs";
import { PAGES_HEADERS, HEADERS_ENTRY } from "../_lib/headers.mjs";

const TAILWIND_CDN = "cdn.tailwindcss.com";

/**
 * Rule 6: the Cloudflare Pages control ENTRIES, which Pages interprets at the
 * ROOT of the uploaded directory (issues #121 and #234).
 *
 * These are not assets. They are configuration of the serving edge, and until
 * #121 the publisher copied them out of a client's site repo without exclusion,
 * without a guard, and without mentioning them anywhere.
 *
 ...
 *   functions     the OTHER way to the same Worker. `wrangler pages deploy`
 *                 compiles this directory into one, so the site's behaviour
 *                 stops being something you can learn by reading the artifact.
 *   _routes.json  decides which paths that Worker sees.
 *   _headers      adds or strips security headers on every response. THE ONLY
 *                 ONE OF THE FIVE THE PUBLISHER NOW WRITES ITSELF (#260); see
 *                 PUBLISHER_AUTHORED_ENTRIES below for what that changes and
 ...
 *                 it was gated.
 *
 * ENTRIES, NOT FILES, AND THAT WORD IS THE FIX #234 LANDED. This map was called
 * PAGES_CONTROL_FILES, its comment said "expressed as files", and rule 6's
 * message asserted "is a Cloudflare Pages control file". The loop underneath was
 * always `fs.existsSync`, which answers about an ENTRY and is true of a
 * directory -- so the check had generalised from the day it was written and only
 * the vocabulary had not. That vocabulary is what kept `functions` off this list
 * for the whole life of the deny-list, and it would have kept the next
 * directory-shaped thing off too: `_worker.js` is itself a directory in Pages'
 * advanced mode, which is why action.yml has excluded `/_worker.js/` all along
 * while this file went on calling it a file. Deny by NAME at the root; report
 * the SHAPE you actually found. Nothing here may be narrowed back to a file
 * test.
 *
 ...
 *
 * ROOT ONLY, and that anchoring is correct rather than incidental. Pages reads
 * these from the deploy directory root and nowhere else, so `assets/_headers` is
 * a file with a name and `assets/functions/` is a folder of assets. Failing
 * either would be a guard inventing a rule.
 */
export const PAGES_CONTROL_ENTRIES = Object.freeze({
  "_worker.js":
  ...
  "functions":
    "is compiled by `wrangler pages deploy` into a Worker in front of this client's site, exactly as a " +
    "_worker.js would be. A Rocket Site is static files with nothing behind them, and code that runs at " +
    "the edge makes the site's behaviour something no reader of these bytes can see",
  "_routes.json": "configures which paths a Pages Function sees, and a Rocket Site has no Pages Function",
  "_headers":
    "rewrites the HTTP headers Cloudflare serves this site with, including its security headers. The " +
    "publisher writes this one itself and the artifact must carry those exact bytes",
  "_redirects":
...
/**
 * The control entries the PUBLISHER writes, mapped to the bytes it writes
 * (issue #260). Everything else in PAGES_CONTROL_ENTRIES must be absent.
 *
 * The direction is the only thing that differs, and the reasoning does not. A
 * Rocket Site is static files on Cloudflare Pages, so a `_headers` at the
 * deploy root is the only mechanism in existence for setting a response header
 * on one -- and until #260 the answer was to emit none of them, which meant
 * every site this product has ever published served with Cloudflare's defaults:
 * frameable, sniffable, and leaking a full referrer off-site. Nothing went red,
 * because the absence of a file nobody writes looks exactly like the absence of
 * a file somebody dropped. That invisibility is the reason this is checked here
 * rather than left to the step that writes it.
 *
 * SO THE GUARD DECIDES THIS ENTRY IN BOTH DIRECTIONS:
 *
 *   missing        the writing step did not run, or ran and wrote nothing. The
 *                  site would publish green with no security headers at all,
 *                  which is the state #260 was filed about. Fails.
 *   not our bytes  something other than the publisher authored the file: a
 *                  build step in the site repository, or a `_headers` that got
 *                  past the rsync deny-list. It can strip a header as easily as
 *                  set one. Fails, and the writing step has already failed for
 *                  the same reason, because either alone is a single point of
 *                  failure exactly as with the symlinks and the other four.
 *   our bytes      what a visitor is served is derivable by reading the
 *                  artifact, which is the property the whole guard rests on.
 *
 * IT IS A COMPARISON AGAINST THE CONSTANT, NOT A SEARCH FOR A HEADER NAME. A
 * file containing `X-Frame-Options` inside a comment, or indented under no path
 * at all, or set to `ALLOWALL`, satisfies every version of this check that
 * greps -- and each of those ships a client site with no clickjacking
 * protection and a green run beside it.
 */
export const PUBLISHER_AUTHORED_ENTRIES = Object.freeze({
  [HEADERS_ENTRY]: PAGES_HEADERS,
});
...
export function runGuardArtifact({ env, logger }) {
  ...
  // ── 6. Cloudflare Pages control entries at the artifact root ──────────
  // existsSync, deliberately: it answers about an ENTRY, so a `functions/`
  // directory and a `_worker.js/` directory are refused by the same line that
  // refuses a `_worker.js` file. The message reports the shape it found rather
  // than asserting one (#234).
  //
  // Both directions since #260: four of these must be absent, and `_headers`
  // must be present carrying the publisher's own bytes. A missing one is the
  // failure that used to be invisible, so it is stated as loudly as an
  // unexpected one.
  for (const [name, why] of Object.entries(PAGES_CONTROL_ENTRIES)) {
    const at = path.join(dir, name);
    const authored = Object.hasOwn(PUBLISHER_AUTHORED_ENTRIES, name);
    ...
    if (!fs.existsSync(at)) {
      if (authored) {
        fail(
          at,
          1,
          `is MISSING from the artifact. The publisher writes it, from a constant, so that every Rocket ` +
            `Site serves its security headers -- Cloudflare Pages has no other mechanism for a response ` +
            `header on a static site. If it is not here the step that writes it did not run, and this ` +
            `site would publish green while serving with Cloudflare's defaults: frameable, sniffable, ` +
            `and sending a full referrer to every site it links out to. That failure is silent on the ` +
            `live site, which is why it is loud here (issue #260).`
        );
      }
      continue;
    }

    const shape = fs.statSync(at).isDirectory() ? "directory" : "file";

    if (authored) {
      const bytes = shape === "file" ? fs.readFileSync(at, "utf8") : null;
      if (bytes === PUBLISHER_AUTHORED_ENTRIES[name]) continue;
      fail(
        at,
        1,
        `is a Cloudflare Pages control ${shape} the publisher did not write. Pages reads it from the ` +
          `root of the uploaded directory, where it ${why}. A copy that is not byte-for-byte the ` +
          `publisher's can strip a header as easily as set one, and it makes what a visitor is served ` +
          `something no reader of these bytes can predict. It is excluded from the upload directory by ` +
          `name, so finding a different one here means it arrived some other way. Delete it from the ` +
    ...
    fail(
      at,
      1,
      `is a Cloudflare Pages control ${shape} and must never be published. Pages reads it from the root ` +
        `of the uploaded directory, where it ${why}. It is excluded from the upload directory by the ` +
  ...
  for (const file of pages) {
    ...
    // ── 10. the embed rule, on the artifact (issue #292, issue #278) ───
    // The rule with no counterpart in findExternalAssets, which is the whole
    // point of it: that function reads <link> and <script>, so every framing
    // element on every page a client hand-edits went unread. _lib/scripting.mjs
    // holds the allowlist, the two places this deliberately differs from the
    // edit guard, and the argument for each.
    for (const problem of embedProblems(content)) {
      fail(file, lineOf(content, problem.index), problem.message);
    }
  }
