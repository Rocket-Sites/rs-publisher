...
composite actions it orchestrates. That is the whole of this repository: 33
files under `.github/`, and nothing else.
...
example, including the inputs and the two secrets it is mapped.
...
- `v1.N.N` are immutable. They are the record of where `v1` was moved to, and
  they are protected against deletion and update.
...
`v1` should resolve to the same commit as the highest `v1.N.N`. If it does not,
the moving tag was moved somewhere other than the release it was cut from.
